...
resource "aws_vpc_security_group_ingress_rule" "RDS_EC2_SG" {
  ...
  security_group_id = aws_security_group.RDS_SG.id # the cidr is every for my ease, i don't want to struggle to much more
  cidr_ipv4         = "0.0.0.0/0"                  #data.aws_vpc.foo[0].cidr_block 
  from_port         = 3306
  ip_protocol       = "tcp"
  to_port           = 3306
}
...
# This is for creating the route for the TGW to Sao Paulo
...
resource "aws_customer_gateway" "aws-to-gcp-cgw1" {
  ...
  bgp_asn    = local.gcp-asn
  ip_address = local.gcp-vpn-ip-1
  type       = "ipsec.1"
...
# Preshared keys for the VPN tunnels, these are randomly generated and outputted as sensitive values
resource "random_password" "tunnel1_psk1" {
  length  = 32
  special = false
}
resource "random_password" "tunnel1_psk2" {
  length  = 32
  special = false
}
# VPN Connection 1
resource "aws_vpn_connection" "aws-to-gcp-vpn1" {
  ...
  transit_gateway_id  = aws_ec2_transit_gateway.shinjuku_tgw01.id
  customer_gateway_id = aws_customer_gateway.aws-to-gcp-cgw1[0].id
  type                = "ipsec.1"
  static_routes_only  = false

  tunnel1_inside_cidr                  = "169.254.0.8/30"
  tunnel1_preshared_key                = random_password.tunnel1_psk1.result # Replace with your pre-shared key 1
  ...
  tunnel2_preshared_key                = random_password.tunnel1_psk2.result # Replace with your pre-shared key 2
...
# Preshared keys for the VPN tunnels, these are randomly generated and outputted as sensitive values
resource "random_password" "tunnel1_psk3" {
  length  = 32
  special = false
}
resource "random_password" "tunnel1_psk4" {
  length  = 32
  special = false
}
# VPN Connection 2
resource "aws_vpn_connection" "aws-to-gcp-vpn2" {
  count               = var.second_workflow_enabled ? 1 : 0
  transit_gateway_id  = aws_ec2_transit_gateway.shinjuku_tgw01.id
  customer_gateway_id = aws_customer_gateway.aws-to-gcp-cgw2[0].id
  type                = "ipsec.1"
  static_routes_only  = false

  tunnel1_inside_cidr                  = "169.254.12.0/30"
  tunnel1_preshared_key                = random_password.tunnel1_psk3.result # Replace with your pre-shared key 3
  tunnel1_ike_versions                 = ["ikev2"]
  tunnel1_phase1_encryption_algorithms = ["AES256"]
  tunnel1_phase1_integrity_algorithms  = ["SHA2-256"]
  ...
  tunnel2_preshared_key                = random_password.tunnel1_psk4.result # Replace with your pre-shared key 4
  tunnel2_ike_versions                 = ["ikev2"]
  tunnel2_phase1_encryption_algorithms = ["AES256"]
  tunnel2_phase1_integrity_algorithms  = ["SHA2-256"]
...
#######################################################################################################   Third Workflow
#      The Third workflow to create the routes



# This is the first vpn association to the transit gateway route tablle
resource "aws_ec2_transit_gateway_route_table_association" "vpc_assoc3" {
  count                          = var.three_workflow_enabled ? 1 : 0
  transit_gateway_attachment_id  = aws_vpn_connection.aws-to-gcp-vpn1[0].transit_gateway_attachment_id
  transit_gateway_route_table_id = aws_ec2_transit_gateway_route_table.rt_a.id
}
# This is the propagation for the first VPN
...
# This is the second vpn association to the transit gateway route tablle
resource "aws_ec2_transit_gateway_route_table_association" "vpc_assoc4" {
  count                          = var.three_workflow_enabled ? 1 : 0
  transit_gateway_attachment_id  = aws_vpn_connection.aws-to-gcp-vpn2[0].transit_gateway_attachment_id
  transit_gateway_route_table_id = aws_ec2_transit_gateway_route_table.rt_a.id
}


# This is the propagation for the second VPN
resource "aws_ec2_transit_gateway_route_table_propagation" "propagation3" {
  count                          = var.three_workflow_enabled ? 1 : 0
  transit_gateway_attachment_id  = aws_vpn_connection.aws-to-gcp-vpn2[0].transit_gateway_attachment_id
  transit_gateway_route_table_id = aws_ec2_transit_gateway_route_table.rt_a.id
}

# This is the route for the VPN in route tables
resource "aws_ec2_transit_gateway_route" "to_region_c_vpc2" {
  count                          = var.three_workflow_enabled ? 1 : 0
  transit_gateway_route_table_id = aws_ec2_transit_gateway_route_table.rt_a.id
...
#   Route table for GCP CIDR block
resource "aws_route" "shinjuku_to_sp_route03" {
  count                  = var.three_workflow_enabled ? 1 : 0
  route_table_id         = aws_route_table.Private.id
  destination_cidr_block = local.gcp_cidr
  transit_gateway_id     = aws_ec2_transit_gateway.shinjuku_tgw01.id
...
#          RUN APPLY TWICE SO IT ADDS THE IPS TO THE ROUTE TABLE, IT IS A FAULT OF TERRAFORM AND THERE ISN'T MUCH THAT CAN BE DONE
#          RUN APPLY TWICE SO IT ADDS THE IPS TO THE ROUTE TABLE, IT IS A FAULT OF TERRAFORM AND THERE ISN'T MUCH THAT CAN BE DONE
#          RUN APPLY TWICE SO IT ADDS THE IPS TO THE ROUTE TABLE, IT IS A FAULT OF TERRAFORM AND THERE ISN'T MUCH THAT CAN BE DONE