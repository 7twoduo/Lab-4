...
# Change the Permission region and                       This is for easier location data.aws_region.current.region   # Use this latervar.secret_region
...
# For replication to the same region
    ...
    Statement = [
      {
        ...
        Resource = [
          "${aws_cloudwatch_log_group.star_alb_log1[0].arn}:*"
        ]
      }
    ]
  })
...
resource "aws_launch_template" "lab-ec2-app-private" {
  image_id               = aws_ami_from_instance.ec2_golden_ami.id
  instance_type          = "t3.micro"
  vpc_security_group_ids = [aws_security_group.EC2_SG.id]
...
resource "aws_autoscaling_group" "bar" {
  ...
  launch_template {
    ...
    version = aws_launch_template.lab-ec2-app-private.latest_version
...
resource "aws_route53_record" "Alb_zone" {
  ...
  name    = "${var.root_alb}.${var.root_domain_name}"
  type    = "A"
...
resource "aws_acm_certificate" "hidden_target_group2" {
  domain_name       = "${var.root_alb}.${var.root_domain_name}"
...
resource "aws_cloudwatch_log_group" "star_alb_log1" {
  ...
  name              = "aws-waf-logs-${local.Environment}-webac"
...
resource "aws_cloudfront_distribution" "main" {
  ...
  logging_config {
    bucket          = aws_s3_bucket.waf-access-logs.bucket_domain_name
    prefix          = var.alb_access_logs_prefix
...
#                  S3 Access Logs
#########################################
resource "aws_s3_bucket" "waf-access-logs" {
  bucket        = "${var.Environment}-waf-bucket-${data.aws_caller_identity.current.account_id}"
  force_destroy = true

  tags = {
    Name        = "${var.Environment}-Waf-bucket"
    Environment = "Production"
  }
}
resource "aws_s3_bucket_ownership_controls" "cf_logs" {
  bucket = aws_s3_bucket.waf-access-logs.id
...
##########################################
#             Bucket Policy to allow Cloudfront to input logs
...
data "aws_iam_policy_document" "cloudfront_logs" {
  statement {
    ...
    resources = [
      "${aws_s3_bucket.waf-access-logs.arn}/*"
    ]
